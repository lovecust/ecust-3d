	...
	for ( var i = 0; i < entities.length; i++ ) {
		...
		switch ( obj.type ) {
			...
			case 'river':
				...
				break;
			case 'prism':
				entity = initPrism( obj );
				break;
			case 'cylinder':
				entity = initCylinder( obj );
				break;
			case 'model':
...
function initRectangle( obj, model ) {
	...
	if ( Array.isArray( obj ) && model ) {
		...
		cube = new THREE.Mesh( geometry, material );
		return handlePosition( cube, obj );
	}
}
...
function initRoad( obj ) {
	...
	var shape = getShape( [ 0, 0, 0, width, height, width, height, 0, 0, 0 ] );
	var geometry = getExtrudeGeometry( shape, { path: path } );
	return new THREE.Mesh( geometry, material );
}
// no model designed yet @D07-04 23:34
function initPrism( obj ) {
	var material = getMaterial( obj );
	var shape = getShape( obj.vertices );
	var p = obj.position;
	var points = getVector3( ( [ 0, 0, 0, 0, 0, obj.height ] ) );
	var geometry = getExtrudeGeometry( shape, { path: new THREE.DirectCurve( points ) } );
	return handlePosition( new THREE.Mesh( geometry, material ), obj );
}
function initCylinder( obj ) {
	var s = obj.size;
	var geometry = new THREE.CylinderGeometry( s.x, s.y, s.z, 32 );
	return handlePosition( new THREE.Mesh( geometry, getMaterial( obj ) ), obj );
}

function getExtrudeGeometry( shape, data ) {
	var config = {
		steps: 200,
		amount: 10,
		bevelEnabled: false,
		bevelThickness: 1.0,
		bevelSize: 1.0,
		extrudePath: data.path
	};
	return new THREE.ExtrudeGeometry( shape, config );
}
